...
def build_critic(fs, fm, init_sigma, init_mean, alpha):
    ...
    y = Conv1D(fm//16, small_fs, strides=5, padding='same', kernel_regularizer=reg,
        bias_regularizer=reg, kernel_initializer=RandomNormal(init_mean,
            init_sigma))(y)
    y = ReLU(negative_slope=alpha)(y)

    y = Conv1D(fm//16, 5, strides=2, padding='same', kernel_regularizer=reg,
        bias_regularizer=reg, kernel_initializer=RandomNormal(init_mean,
            init_sigma))(y)
    y = ReLU(negative_slope=alpha)(y)

    y = Conv1D(fm//16, 3, strides=2, padding='same', kernel_regularizer=reg,
        bias_regularizer=reg, kernel_initializer=RandomNormal(init_mean,
            init_sigma))(y)
    y = ReLU(negative_slope=alpha)(y)

...
class WGANGP():
    ...
    def __init__(self,gen,critic,noise_dim,n_critic,batch_size,gen_lr,critic_lr,text,set_train,set_run):
        ...
        if (set_run == False) or (set_train == False):
            ####self.current_run = self.get_run()
            ####self.dir_path = f"runs/TRAIN{self.set_train}/{self.current_run}/"
            ####print(f"\nCreating new directories for run: {self.current_run}\n")
            ####os.mkdir(self.dir_path)
            print(f"\n TRAIN and RUN numbers must be specified! Exiting.\n")
            exit()
        elif type(set_run) == type(2):
            #Read train and run numbers
            self.current_train = set_train
            self.current_run   = set_run
            #Makes train folder
            self.dir_path = f"runs/TRAIN{self.current_train}/"
            if not os.path.exists(self.dir_path):
                print(f"\nCreating new directories for run: TRAIN{self.current_train}/\n")
                os.makedirs(self.dir_path)
            #Makes run folder
            self.dir_path = self.dir_path+f"{self.current_run}/"
            if not os.path.exists(self.dir_path):
                print(f"\nCreating new directories for run: TRAIN{self.current_train}/{self.current_run}/\n")
                os.makedirs(self.dir_path)
            else:
    ...
    ####def get_run(self):
    ####    runs = glob.glob('runs/TRAIN{self.set_train}/*/')
    ...
    def compute_pdf_real(self, db_real):
        ...
        XCOMPONENT = slice(0,1)
        velo = db_real[:,:,XCOMPONENT] # WE WITHOUT EXTREMES
    ...
    def plot_statistics(self, mini_db, epoch, pdf_ve_real, pdf_ac_real):
        #Remove Borders
        XCOMPONENT = slice(0,1)
        velo = mini_db[:,100:1900,XCOMPONENT] # WE WITHOUT EXTREMES
        #Compute first der
        acce = np.gradient(velo,axis=1)
...
if __name__ == '__main__' :
    ...
    while len(sys.argv) > 1:
        if sys.argv[1] == "-h":
            print((f"usage: wgan.py [--gen_iters <{gen_iters}>]"
                   ...
                   f" [--critic_lr <{critic_lr}>] [--set_train <{train_number}>] [--set_run <run>]"))
            exit()
        ...
        elif sys.argv[1] == "--set_train":
            _ = sys.argv.pop(1)
            set_train = int(sys.argv.pop(1))
    ...
    if option_load:
        ...
        run    = load[0]
        number = load[1]
        path_gen = f'runs/TRAIN{set_train}/{run}/{number}_gen.h5'
        path_critic = f'runs/TRAIN{set_train}/{run}/{number}_critic.h5'
    ...
    wgan = WGANGP(gen, critic, NOISE_DIM, ncritic, batch_size,
                  gen_lr, critic_lr, text, set_train, set_run)

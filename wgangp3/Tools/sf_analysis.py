...
if "-h" in sys.argv:
    print("usage: sf_analysis.py <train> <run> <number> [--no_plot]")
    exit()
...
# FIND SF.DAT FILES
if not mean_over_train:
    read_file = f'runs/TRAIN{train}/{run}/HighStat/sf_{number}_me_*.dat'
else:
    read_file = f'runs/TOT-STAT/LINKS/sf_{number}_me_*.dat'

read_file_real = '../data/real/sfr.dat'
...
## 
##     fig.tight_layout()
##     img_file = osp.join(save_path, 'fig_sf.png')
